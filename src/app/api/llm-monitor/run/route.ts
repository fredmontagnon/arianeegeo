      ...
      if (totalResultCount && totalResultCount >= queries.length * 2) {
        try {
          ...
          if (allResults && allResults.length > 0) {
            ...
            const totalWeightRun = llmScoreEntries.reduce((sum, { llm }) => sum + (LLM_MARKET_WEIGHTS[llm as LLMName] || 0), 0);
            const scoreGlobal = llmScoreEntries.length > 0 && totalWeightRun > 0
              ? Math.round(llmScoreEntries.reduce((sum, { llm, score }) => sum + score * (LLM_MARKET_WEIGHTS[llm as LLMName] || 0), 0) / totalWeightRun)
              : 0;
